...
public class App {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Client from city: " + client.getCity());  // Выводим город
    ...
    public void logEvents(ApplicationContext ctx) {
        ...
        event = ctx.getBean(Event.class);
        logEvent(EventType.WARNING, event, "Warning: low disk space");

        event = ctx.getBean(Event.class);
        logEvent(EventType.DEBUG, event, "Debug: variable value = 42");

        event = ctx.getBean(Event.class);
        logEvent(EventType.CRITICAL, event, "Critical: system failure imminent");
    }
